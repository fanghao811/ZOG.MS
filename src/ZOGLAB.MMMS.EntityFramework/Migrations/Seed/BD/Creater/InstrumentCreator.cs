...
namespace ZOGLAB.MMMS.Migrations.Seed.BD
{
    public class InstrumentCreator
    {
        ...
        private void CreateInstruments()
        {
            ...
            var checkListW = checkLists.Where(c => new long[3] { 4, 5, 6 }.Contains(c.Id)).ToList();
            var checkListB = checkLists.Where(c => new long[3] { 7, 8, 9 }.Contains(c.Id)).ToList();
            ...
            if (defaultUnit == null)
            {
                var seedUnits = new List<BD_Instrument>
                {
                    ...
                    new BD_Instrument{
                        ...
                        Manufacturer ="杭州仪器厂",Mark="测试时间：12/24/2018",CheckTypes=checkListW,MeteorTypes=meteorTypesW
                    },
                    new BD_Instrument{
