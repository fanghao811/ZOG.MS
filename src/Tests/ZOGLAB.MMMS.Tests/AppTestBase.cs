...
using ZOGLAB.MMMS.Tests.TestDatas;
...
namespace ZOGLAB.MMMS.Tests
{
    ...
    /// Provides methods to easily work with <see cref="MMMSDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<MMMSTestModule>
    {
        ...
        protected void UsingDbContext(Action<MMMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MMMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MMMSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MMMSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MMMSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MMMSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MMMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MMMSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MMMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MMMSDbContext, Task<T>> func)
        {
            T result;
