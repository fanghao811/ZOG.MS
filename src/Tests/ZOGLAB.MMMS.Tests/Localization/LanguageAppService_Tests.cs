...
using ZOGLAB.MMMS.Localization.Dto;
using ZOGLAB.MMMS.Migrations.Seed.Host;
using Shouldly;
using Xunit;

namespace ZOGLAB.MMMS.Tests.Localization
{
    public class LanguageAppService_Tests : AppTestBase
    {
        ...
        public async Task UpdateLanguageText()
        {
            await _languageAppService.UpdateLanguageText(
                ...
                {
                    SourceName = MMMSConsts.LocalizationSourceName,
                    LanguageName = "en",
                    Key = "Save",
            ...
            var newValue = Resolve<ILocalizationManager>()
                .GetString(
                    MMMSConsts.LocalizationSourceName,
                    "Save",
